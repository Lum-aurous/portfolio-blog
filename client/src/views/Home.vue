...
import logger from '@/utils/logger' // 导入新日志工具
import envConfig from '@/utils/env' // 导入环境配置

const userStore = useUserStore()
const myCustomUrl = ref(null)

// ==================== 基础状态 ====================
const isSettingsOpen = ref(false)
// 默认给一个空，防止闪烁
const wallpaperSource = ref('website')
...
const hasUserCustomWallpaper = computed(() => userHasCustom.value)
...
const loadGlobalWallpaper = async () => {
    try {
        logger.debug('正在加载全局壁纸配置...')
        const res = await fetch('/api/wallpaper/global')
        if (!res.ok) throw new Error('Network response was not ok')
        const data = await res.json()
        globalConfig.value = data
        logger.success('全局配置加载完成')
    } catch (err) {
        logger.error('后端全局壁纸接口异常，使用本地兜底', err)
        // 兜底数据
        globalConfig.value = {
            mode: 'website',
            dailyUrl: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-4.0.3&auto=format&fit=crop&w=3000&q=80',
            websiteUrl: 'https://images.unsplash.com/photo-1493246507139-91e8fad9978e?ixlib=rb-4.0.3&auto=format&fit=crop&w=2940&q=80',
            randomUrls: []
        }
    }
}
...
const loadUserWallpaper = async () => {
    if (!userStore.isLoggedIn) {
        logger.debug('用户未登录，跳过加载个人壁纸')
        return null
    }

    try {
        logger.user('加载用户壁纸', '用户ID:', userStore.user.id)

        const res = await fetch(`/api/wallpaper/user?userId=${userStore.user.id}`)

        if (!res.ok) {
            logger.warn('加载个人壁纸失败，状态码:', res.status)
            return null
        }

        const data = await res.json()
        logger.debug('用户壁纸数据:', data)

        if (data.hasCustom && data.url) {
            logger.success('找到用户自定义壁纸:', data.url)

            // 注意：这里的数据可能已经包含前导斜杠
            const cleanUrl = data.url.startsWith('/') ? data.url : '/' + data.url
            myCustomUrl.value = cleanUrl
            userHasCustom.value = true

            return cleanUrl
        }
    } catch (err) {
        logger.error('加载个人壁纸异常:', err)
    }
...
const wallpaperStyle = computed(() => {
    logger.wallpaper('计算壁纸样式', 'URL:', currentWallpaper.value)
...
// 切换壁纸显示（简化版）
const changeWallpaper = (mode) => {
    logger.wallpaper('切换壁纸模式', mode)
    logger.debug('myCustomUrl:', myCustomUrl.value)
    ...
    if (mode === 'userCustom') {
        if (myCustomUrl.value) {
            ...
            const encodedUrl = encodeURI(myCustomUrl.value) + `?t=${timestamp}`
            currentWallpaper.value = encodedUrl
            logger.success('显示自定义壁纸:', currentWallpaper.value)
        } else {
            logger.warn('无自定义壁纸，回退到网站背景')
            wallpaperSource.value = 'website'
            currentWallpaper.value = globalConfig.value.websiteUrl
        }
        return
    }
    ...
    // 其他模式
    if (mode === 'daily') {
        currentWallpaper.value = globalConfig.value.dailyUrl
    } else if (mode === 'random') {
        const list = globalConfig.value.randomUrls || []
        if (list.length > 0) {
            const randomIndex = Math.floor(Math.random() * list.length)
            currentWallpaper.value = list[randomIndex]
        } else {
            currentWallpaper.value = globalConfig.value.websiteUrl
        }
    ...
    logger.success('壁纸已切换为:', currentWallpaper.value)
}

// ✅ 简化预加载函数
const preloadImage = (url) => {
    return new Promise((resolve) => {
        logger.debug('开始预加载图片:', url)
        const img = new Image()
        ...
        img.onload = () => {
            logger.success('图片预加载完成:', finalUrl)
            resolve(img)
        }

        img.onerror = (err) => {
            logger.error('图片预加载失败:', err)
            logger.debug('图片预加载失败，但继续执行')
            // 即使加载失败，也继续执行
            resolve(null)
        }
...
// 4. 监听用户切换操作
watch(wallpaperSource, (newVal) => {
    // 只要用户切了模式，就记住它
    localStorage.setItem('preferredWallpaperMode', newVal)

    // ✅ 修复：如果已经是自定义模式且有图片，跳过重复切换
    if (newVal === 'userCustom' && currentWallpaper.value && currentWallpaper.value.includes('uploads')) {
        logger.debug('已经是自定义模式且有图片，跳过重复切换')
        return
    }

    // 如果切到了自定义，但其实没图，就提示一下
    if (newVal === 'userCustom' && !userHasCustom.value) {
        // 这种情况下通常是用户点击了按钮但还没上传
        // 这里不做强行重置，允许用户停留在该状态等待上传
        logger.debug('用户选择了自定义模式，但尚未上传壁纸')
    } else {
...
const handleFileUpload = async (e) => {
    ...
    try {
        ...
        if (data.success) {
            ...
            logger.success('上传成功！新URL:', newUrl)

            // ✅ 1. 先清空 currentWallpaper，强制重新渲染
            currentWallpaper.value = ''

            // ✅ 2. 使用 nextTick 确保 DOM 更新后再设置新值
            await nextTick()
            ...
            // ✅ 4. 更新 myCustomUrl
            myCustomUrl.value = newUrl
            userHasCustom.value = true

            // ✅ 5. 保存到 localStorage
            localStorage.setItem('preferredWallpaperMode', 'userCustom')
            localStorage.setItem('tempCustomUrl', newUrl)

            // ✅ 6. 直接设置 currentWallpaper（不通过 changeWallpaper）
            const timestamp = Date.now()
            const encodedUrl = encodeURI(newUrl) + `?t=${timestamp}`
            currentWallpaper.value = encodedUrl

            logger.success('直接设置壁纸URL:', currentWallpaper.value)

            // ✅ 7. 确保切换到自定义模式
            wallpaperSource.value = 'userCustom'

            logger.success('前端状态已更新')
            alert('🎉 壁纸上传成功！')
        } else {
            logger.error('上传失败:', data.error)
            alert('上传失败：' + (data.error || '未知错误'))
        }
    } catch (err) {
        logger.error('网络请求失败:', err)
        alert('上传失败，请检查网络: ' + err.message)
    } finally {
        // 清空 input，允许重复选择同一文件
        e.target.value = ''
    }
...
watch([use12Hour, showSeconds, showLunar], updateTime)
...
onMounted(async () => {
    ...
    // 2. 加载全局壁纸配置
    logger.debug('加载全局壁纸配置...')
    await loadGlobalWallpaper()
    logger.debug('全局配置加载完成:', globalConfig.value)

    // 3. 如果用户已登录，加载个人壁纸
    let customUrl = null
    if (userStore.isLoggedIn) {
        logger.user('用户已登录', 'ID:', userStore.user.id)
        customUrl = await loadUserWallpaper()

        if (customUrl) {
            logger.success('找到用户自定义壁纸:', customUrl)
            // ✅ 修复: 保存原始 URL
            localStorage.setItem('tempCustomUrl', customUrl)
            userHasCustom.value = true
        } else {
            logger.debug('用户无自定义壁纸')
            userHasCustom.value = false
        }
    } else {
        logger.debug('用户未登录')
    ...
    logger.debug('本地偏好设置:', savedMode)

    if (savedMode === 'userCustom' && customUrl) {
        // 用户之前选择自定义，且有壁纸
        wallpaperSource.value = 'userCustom'
        const timestamp = Date.now()
        currentWallpaper.value = customUrl + '?t=' + timestamp
        logger.success('恢复用户自定义壁纸')
    } else if (savedMode === 'userCustom' && !customUrl) {
        // 用户之前选择自定义，但壁纸不存在
        logger.warn('之前的自定义壁纸已失效，回退到网站背景')
        wallpaperSource.value = 'website'
        localStorage.setItem('preferredWallpaperMode', 'website')
        changeWallpaper('website')
    } else if (savedMode) {
        // 其他模式
        wallpaperSource.value = savedMode
        changeWallpaper(savedMode)
        logger.success('恢复用户偏好模式:', savedMode)
    } else {
        // 默认模式
        const defaultMode = globalConfig.value?.mode || 'website'
        wallpaperSource.value = defaultMode
        changeWallpaper(defaultMode)
        logger.success('使用全局默认模式:', defaultMode)
    }

    logger.info('初始化完成！当前模式:', wallpaperSource.value)
    logger.debug('当前壁纸URL:', currentWallpaper.value)
})
...
<template>
    <div class="home-container" :style="{ '--bg-blur': wallpaperBlur + 'px' }">
        <!-- 使用 wallpaperStyle 计算属性 -->
        <div class="bg-layer" :style="wallpaperStyle">
            <div class="bg-mask" v-if="wallpaperMask"></div>
        </div>
        <!-- 页面内容 -->
                    ...
                    <div class="settings-group">
                        ...
                        <div class="setting-row">
                            <div class="row-info">
                                ...
                                <div class="row-desc">刷新页面后生效</div>
                            </div>
