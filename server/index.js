...
app.post('/api/articles', (req, res) => {
    ...
    const sql = 'INSERT INTO articles (title, summary, content, cover_image) VALUES (?, ?, ?, ?)';
...
app.post('/api/register', (req, res) => {
    ...
    const checkSql = 'SELECT * FROM users WHERE username = ?';
    db.query(checkSql, [username], (err, results) => {
        if (results.length > 0) return res.status(409).json({ message: '用户名已被占用' });
...
app.post('/api/login', (req, res) => {
    ...
    db.query(sql, [username], (err, results) => {
        ...
        // 2. 🔐 核心改变：使用 bcrypt.compareSync 比对密码
        // 拿用户输入的明文 password 和 数据库里的乱码 user.password 比对
...
app.post('/api/comments', (req, res) => {
    ...
    const sql = 'INSERT INTO comments (article_id, nickname, content) VALUES (?, ?, ?)';
